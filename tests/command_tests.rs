...
mod command_unit_tests {
    ...
    fn test_environment_setup() {
        ...
        fs::write(&readme_path, "# Documentation\n\nThis is documentation.\n\nMore content here.").unwrap();
...
"#;
        ...
        let start_index = readme_lines.iter().position(|&line| line.contains("```rust")).unwrap();
        let end_index = readme_lines.iter().skip(start_index + 1).position(|&line| line.contains("```")).unwrap() + start_index + 1;
    ...
    fn test_file_discovery() {
        ...
        for entry in entries {
            if entry.ends_with(".md") || entry.to_lowercase().contains("readme") || 
